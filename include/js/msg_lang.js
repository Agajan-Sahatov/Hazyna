...
request.open('GET', '../include/json/msg_lang.json');
	...
	if ((request.readyState===4) && (request.status===200)) {
		...
		for (var key in item){
			if(document.getElementById(key) !== null && item[key] !== ""){
				...
				else if(document.getElementById(key).getAttribute('type') === 'text'){
					document.getElementById(key).setAttribute('placeholder', item[key]);
				}else{
